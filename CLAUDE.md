...
- Rocket/Datastar gotchas (learned the hard way):
  - `observeProps` only fires when the *decoded* value changes (`#setProp` returns early on `Object.is`). So `checked="false"` written onto an element that never had the attribute is silent, and a local edit survives a server clear — verified on `sb-toggle`. Where the server owns a boolean or must be able to clear a value, watch the attribute itself (`new MutationObserver(…).observe(host, { attributeFilter: ['open'] })`) and apply the same server-wins logic there.
